...
async function verify() {
  console.log('\n🔍 VERIFICANDO NOCHES 2025 EN SUPABASE...\n');

  const { data: bookings } = await supabase
    .from('bookings')
    .select('guest_name, check_in, check_out, nights')
    .eq('tenant_id', GITA_TENANT_ID)
    .gte('check_in', '2025-01-01')
    .lt('check_in', '2026-01-01')
    .order('check_in');

  console.log('═'.repeat(80));
  console.log('BOOKINGS 2025 (26 total):');
  console.log('═'.repeat(80));
  ...
  console.log('═'.repeat(80));
  console.log(`TOTAL NOCHES: ${totalNights}`);
  console.log(`TOTAL BOOKINGS: ${bookings.length}`);
  console.log('═'.repeat(80));
  console.log('\n');

  if (totalNights !== 79) {
    console.log(`❌ ERROR: Debería ser 79 noches, pero es ${totalNights}`);
  } else {
