...
const WorkflowCard = ({ name, trigger, actions, status, lastRun, runsToday, icon: Icon }) => (
  <div className="bg-white/95 backdrop-blur-sm rounded-3xl p-6 border-2 border-white/50 hover:shadow-2xl shadow-xl transition-all">
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h4 className="font-bold text-orange-600 text-lg mb-1">{name}</h4>
    ...
    <div className="flex items-center justify-between pt-4 border-t-2 border-gray-100">
      <div className="flex items-center gap-4">
        <div>
          ...
          <p className="text-sm font-bold text-orange-600">{lastRun}</p>
        </div>
        <div>
          <p className="text-xs text-gray-500 mb-1">Runs Today</p>
          <p className="text-sm font-bold text-orange-600">{runsToday}</p>
        </div>
      </div>
      <button className="p-2 bg-orange-50 text-orange-600 rounded-xl hover:bg-orange-100 transition-colors">
