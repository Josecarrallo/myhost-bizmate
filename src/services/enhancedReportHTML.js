// Generate complete HTML for ENHANCED business report
export function generateEnhancedReportHTML(ownerName, propertyName, currency, data, osirisAnalysis, startDate, endDate) {
  console.log('✅ generateEnhancedReportHTML v3.0 - ENHANCED with ADR, RevPAR, Monthly Breakdown & Financial Statement');
  const { metrics, channels, propertyMetrics, monthlyData, monthlyStatements } = data;
...
<body>
    ...
    <div class="page">
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-box">
                ...
                <div class="metric-subtitle">${metrics.avgStayLength.toFixed(1)} nights avg</div>
            </div>
        </div>

        <!-- KEY METRICS - Row 2 (Enhanced) -->
        <div class="metrics-grid" style="margin-top: 12px;">
            <div class="metric-box">
                <div class="metric-label">ADR</div>
                <div class="metric-value">${formatCurrency(metrics.adr, currency)}</div>
                <div class="metric-subtitle">Average Daily Rate</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">RevPAR</div>
                <div class="metric-value">${formatCurrency(metrics.revpar, currency)}</div>
                <div class="metric-subtitle">Rev Per Avail Room</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Cancellations</div>
                <div class="metric-value">${metrics.totalCancellations}</div>
                <div class="metric-subtitle">${metrics.cancellationRate.toFixed(1)}% cancel rate</div>
            </div>
            <div class="metric-box">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr ${bgStyle}>
                        ...
                        <td>${formatCurrency(prop.adr || 0, currency)}</td>
                        <td>${formatCurrency(prop.revpar || 0, currency)}</td>
                    </tr>
                      `;
                    }).join('')}
                </tbody>
    ...
    <!-- PAGE 3: MONTHLY PERFORMANCE BREAKDOWN -->
    <div class="page">
        <div class="header">
        ...
        <div class="metrics-grid">
            <div class="metric-box">
                ...
                <div class="metric-value">${monthlyData ? monthlyData.length : 0}</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Avg Monthly Revenue</div>
                <div class="metric-value">${monthlyData && monthlyData.length > 0 ? formatCurrency(monthlyData.reduce((sum, m) => sum + m.revenue, 0) / monthlyData.length, currency) : '0'}</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Avg Monthly Bookings</div>
                <div class="metric-value">${monthlyData && monthlyData.length > 0 ? Math.round(monthlyData.reduce((sum, m) => sum + m.bookings, 0) / monthlyData.length) : 0}</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Avg Monthly ADR</div>
                <div class="metric-value">${monthlyData && monthlyData.length > 0 ? formatCurrency(monthlyData.reduce((sum, m) => sum + m.adr, 0) / monthlyData.length, currency) : '0'}</div>
            </div>
        </div>

        <!-- MONTHLY PERFORMANCE TABLE -->
        <div class="section">
            <div class="section-title">Month-by-Month Performance</div>
            <table class="data-table">
                <thead>
                    <tr>
                        <th>Month</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>${formatCurrency(month.adr, currency)}</td>
                    </tr>
                    `).join('') : '<tr><td colspan="6" style="text-align: center;">No monthly data available</td></tr>'}
                </tbody>
            </table>
        </div>
        ...
        <div class="section">
            <div class="section-title">Monthly Channel Distribution</div>
            ${monthlyData && monthlyData.length > 0 ? monthlyData.slice(0, 6).map(month => {
                const channels = Object.entries(month.channels || {});
                if (channels.length === 0) return '';
                return `
                <div class="summary-box">
                    <h3>${month.month} - Channels</h3>
                    <table class="data-table" style="margin-top: 10px; margin-bottom: 0;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>${data.bookings}</td>
                                <td>${formatCurrencyFull(data.revenue, currency)}</td>
                                <td>${((data.revenue / month.revenue) * 100).toFixed(1)}%</td>
                            </tr>
                            `).join('')}
                        </tbody>
                    </table>
        ...
        <div class="footer">
            ${propertyName} - Monthly Performance Breakdown | Page 3
        </div>
    </div>

    <!-- PAGE 4: MONTHLY OWNER FINANCIAL STATEMENT -->
    <div class="page">
        <div class="header">
            <h1>MONTHLY OWNER FINANCIAL STATEMENT</h1>
            <div class="subtitle">${propertyName} | ${START_DATE} to ${END_DATE}</div>
        </div>
        ...
        <div class="metrics-grid">
            <div class="metric-box">
                ...
                <div class="metric-value">${monthlyStatements && monthlyStatements.length > 0 ? formatCurrency(monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0), currency) : '0'}</div>
                ${currency === 'IDR' && monthlyStatements && monthlyStatements.length > 0 ? `<div class="metric-subtitle">~$${Math.round(monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0) / 16000).toLocaleString()}</div>` : ''}
            </div>
            <div class="metric-box">
                <div class="metric-label">Total OTA Commission</div>
                <div class="metric-value">${monthlyStatements && monthlyStatements.length > 0 ? formatCurrency(monthlyStatements.reduce((sum, m) => sum + m.otaCommission, 0), currency) : '0'}</div>
                <div class="metric-subtitle">15% of OTA bookings</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Total Management Fee</div>
                <div class="metric-value">${monthlyStatements && monthlyStatements.length > 0 ? formatCurrency(monthlyStatements.reduce((sum, m) => sum + m.managementFee, 0), currency) : '0'}</div>
                <div class="metric-subtitle">20% of gross revenue</div>
            </div>
            <div class="metric-box">
                <div class="metric-label">Total Net Payout</div>
                <div class="metric-value">${monthlyStatements && monthlyStatements.length > 0 ? formatCurrency(monthlyStatements.reduce((sum, m) => sum + m.netPayout, 0), currency) : '0'}</div>
                <div class="metric-subtitle">Your earnings</div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <table class="data-table">
                ...
                <tbody>
                    ${monthlyStatements && monthlyStatements.length > 0 ? monthlyStatements.map(month => `
                    <tr>
                        <td><strong>${month.month}</strong></td>
                        <td>${month.bookings}</td>
                    ...
                    `).join('') : '<tr><td colspan="6" style="text-align: center;">No financial data available</td></tr>'}
                    ${monthlyStatements && monthlyStatements.length > 0 ? `
                    <tr style="background: #ffedd5; font-weight: 700;">
                        <td>TOTAL</td>
                        <td>${monthlyStatements.reduce((sum, m) => sum + m.bookings, 0)}</td>
                        <td class="highlight-number">${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0), currency)}</td>
                        <td>${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.otaCommission, 0), currency)}</td>
                        <td>${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.managementFee, 0), currency)}</td>
                        <td class="highlight-number">${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.netPayout, 0), currency)}</td>
                    </tr>
                    ` : ''}
                </tbody>
            </table>
            ...
            <div class="info-grid">
                <div class="info-card">
                    ...
                    <div class="info-text" style="line-height: 1.8;">
                        ${monthlyStatements && monthlyStatements.length > 0 ? `
                        <div style="margin-bottom: 12px;">From <strong>${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0), currency)}</strong> gross revenue:</div>
                        <div style="padding-left: 8px;">
                            <div style="margin-bottom: 8px;">• <strong>OTA Commission:</strong> ${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.otaCommission, 0), currency)} <span style="color: #f97316;">(${((monthlyStatements.reduce((sum, m) => sum + m.otaCommission, 0) / monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0)) * 100).toFixed(1)}%)</span></div>
                            <div style="margin-bottom: 8px;">• <strong>Management Fee:</strong> ${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.managementFee, 0), currency)} <span style="color: #f97316;">(${((monthlyStatements.reduce((sum, m) => sum + m.managementFee, 0) / monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0)) * 100).toFixed(1)}%)</span></div>
                            <div>• <strong>Net Payout:</strong> ${formatCurrencyFull(monthlyStatements.reduce((sum, m) => sum + m.netPayout, 0), currency)} <span style="color: #10b981; font-weight: 600;">(${((monthlyStatements.reduce((sum, m) => sum + m.netPayout, 0) / monthlyStatements.reduce((sum, m) => sum + m.grossRevenue, 0)) * 100).toFixed(1)}%)</span></div>
                        </div>
                        ` : 'No data available'}
                    </div>
                </div>
                <div class="info-card">
                    <div class="info-title">Fee Breakdown</div>
                    <div class="info-text" style="line-height: 1.8;">
                        <div style="padding-left: 8px;">
                            <div style="margin-bottom: 8px;">• <strong>OTA Bookings:</strong> 15% commission + 20% management fee</div>
                            <div style="margin-bottom: 12px;">• <strong>Direct Bookings:</strong> 0% commission + 20% management fee</div>
                            <div style="background: #fff3e0; padding: 10px; border-left: 4px solid #f97316; border-radius: 4px; margin-top: 12px;">
                                <strong style="color: #f97316;">💡 Tip:</strong> Increasing direct bookings saves 15% in OTA fees.
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="footer">
            ${propertyName} - Monthly Owner Financial Statement | Page 4
        </div>
    </div>
