-- =====================================================
-- FIX: Occupancy based on actual booking period
-- Instead of using p_start_date to p_end_date,
-- use MIN(check_in) to MAX(check_out) from actual bookings
-- =====================================================
...
DECLARE
  ...
  v_booking_period_start DATE;
  v_booking_period_end DATE;
BEGIN
  -- Calculate actual booking period (first check-in to last check-out)
  SELECT
    MIN(check_in),
    MAX(check_out)
  INTO v_booking_period_start, v_booking_period_end
  FROM bookings
  ...
  -- Calculate available nights based on actual booking period
  IF v_booking_period_start IS NOT NULL AND v_booking_period_end IS NOT NULL THEN
    v_total_available_nights := (v_booking_period_end - v_booking_period_start);
  ELSE
    v_total_available_nights := (p_end_date - p_start_date + 1);
  END IF;
